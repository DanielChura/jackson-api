...
public class OrderController {
    ...
    @GetMapping("/user/{id}")
    public ResponseEntity<PagedResponse<OrderResponse>> getByUserId(@PathVariable UUID id, Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK).body(PagedResponse.from(orderService.getByUserId(id, pageable)));
    }

    @GetMapping("/me")
    public ResponseEntity<PagedResponse<OrderResponse>> getMyOrders(
            @PageableDefault(page = 0, size = 10) @SortDefault(sort = "orderedAt", direction = Direction.DESC) Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK).body(PagedResponse.from(orderService.getMineOrders(pageable)));
    }
